from datetime import datetime
from typing import Any

from sqlalchemy import BigInteger, DateTime, Integer, JSON, SmallInteger, String, UniqueConstraint, func
from sqlalchemy.orm import Mapped, mapped_column

from app.core.database import Base

ID_TYPE = BigInteger().with_variant(Integer, "sqlite")


class StoreDesignPage(Base):
    __tablename__ = "store_design_page"
    __table_args__ = (UniqueConstraint("company_id", "diy_id", name="uk_page_company_diy"),)

    id: Mapped[int] = mapped_column(ID_TYPE, primary_key=True, autoincrement=True)
    diy_id: Mapped[int] = mapped_column(BigInteger, index=True)
    company_id: Mapped[int] = mapped_column(BigInteger, index=True)
    type: Mapped[str] = mapped_column(String(32), default="home_page")
    page_name: Mapped[str] = mapped_column(String(255))
    datas: Mapped[list[dict[str, Any]]] = mapped_column(JSON, default=list)
    other_company_show: Mapped[int] = mapped_column(SmallInteger, default=0)
    page_info: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    member_level: Mapped[list[Any]] = mapped_column(JSON, default=list)
    level: Mapped[list[Any]] = mapped_column(JSON, default=list)
    status: Mapped[int] = mapped_column(SmallInteger, default=1)
    page_sort: Mapped[int] = mapped_column(SmallInteger, default=2)
    page_scene: Mapped[int] = mapped_column(SmallInteger, default=2)
    top_id: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    foot_type: Mapped[int] = mapped_column(SmallInteger, default=1)
    foot_id: Mapped[dict[str, Any]] = mapped_column(JSON, default=dict)
    page_type: Mapped[str] = mapped_column(String(32), default="2")
    schema_version: Mapped[int] = mapped_column(default=1)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
    deleted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
